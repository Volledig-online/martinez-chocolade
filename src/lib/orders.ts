  ...
  return executeQuery(async dbPool => {
    ...
    const result = await dbPool
      ...
      .input('endDate', sql.DateTime, threeDaysFromNow).query<DatabaseOrder>(`
        ...
        AND ([WMS_Status] = 'New' OR [WMS_Status] = '' OR [WMS_Status] IS NULL)
...
export async function getFutureOrders(): Promise<Order[]> {
  return executeQuery(async dbPool => {
    ...
    // Fetch all orders from today onwards to account for business rule adjustments
    const result = await dbPool
      .request()
      .input('startDate', sql.DateTime, today).query<DatabaseOrder>(`
        SELECT * FROM [dbo].[_AB_OrderRegel_View]
        WHERE [Leverdatum] >= @startDate
        AND ([WMS_Status] = 'New' OR [WMS_Status] = '' OR [WMS_Status] IS NULL)
